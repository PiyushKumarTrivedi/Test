...
function startEmulator(answers) {
  try {
    ...
    logMessage(MESSAGE_TYPES.NOTIFY, `${answers.emulator} is being started. You can close the device with 'X'-button on the toolbar of the device.`); // Start the emulator

    const runOnEmulator = (0, _child_process.spawnSync)((0, _path.normalize)((0, _path.join)(process.env.ANDROID_HOME, '/emulator/emulator')), ['-avd', answers.emulator, answers.wipeData === 'Yes' ? '-wipe-data' : '-no-snapshot']);
...
function getEmulators() {
  return (0, _child_process.execFileSync)('emulator', ['-list-avds'], {
    encoding: 'utf8'
  }).replace(/\n$/, '').split('\n');
}
/**
